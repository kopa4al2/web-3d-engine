...
class MeshTweakPane {
  ...
  addMesh(entity: string, mesh: Mesh, transform: Transform) {
    ...
    const transformMonitor = {
      get data() {
        return `Target position: [${arrToFixed(transform.targetTransform.position, 2)}]
Local position: [${arrToFixed(transform.localTransform.position, 2)}, 
World position: [${arrToFixed(transform.worldTransform.position, 2)}]
Target rotation: [${arrToFixed(transform.targetTransform.rotation, 2)}]
