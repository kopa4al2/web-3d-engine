...
export default class HDRLoader {
  ...
  public static async loadHDRImages(rootUrl: string, relativeUrls: string[]): Promise<HDRImageData[]> {
    return await Promise.all(relativeUrls.map(relative =>
      fetch(rootUrl + relative, { cache: 'force-cache' })
        .then(data => data.arrayBuffer())
        .then(parseHDR)));
  }

  public static async loadHDRImagesV2(rootUrl: string, relativeUrls: string[]): Promise<HDRImage[]> {
    // const allData: HDRImage[] = [];
    return Promise.all(relativeUrls.map(relative => fetch(rootUrl + relative, { cache: 'force-cache' })
      .then(data => data.arrayBuffer())
      .then(data => {
    ...
    /* for (const relativeUrl of relativeUrls) {
       const data = await fetch(rootUrl + relativeUrl, { cache: 'force-cache' })
         .then(data => data.arrayBuffer());
