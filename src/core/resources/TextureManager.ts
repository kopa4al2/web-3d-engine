...
const usedType: keyof typeof ImageChannelRange = 'uint8';
...
export default class TextureManager {
  ...
  private async _loadCubeMap(path: string, names: string[], isHdr = false) {
    const images = isHdr
      ...
      : await Promise.all(names.map(relative => this.loadImageAsBitMap(path + relative)));

    // const width = images[0].width;
    // const height = images[0].height;

    const width = isHdr ? ( images[0] as HDRImageData ).shape[0] : ( images[0] as ImageBitmap ).width;
    const height = isHdr ? ( images[0] as HDRImageData ).shape[1] : ( images[0] as ImageBitmap ).height;
    const textureId = this.getEnvironmentMap();

    images.forEach((img, idx) => {
      const imageData = img instanceof ImageBitmap ? img : img.data;
  ...
  private async createTextureFromValues(label: string, data: Uint8ClampedArray, imgWidth: number, imgHeight: number): Promise<Texture> {
    if (this.cachedTextures.get(label)) {
      console.debug(`Texture ${label} is present in cache, loading from there`);
  ...
  private _addPreloadedToGlobalTexture(id: string, image: TextureData) {
    if (this.cachedTextures.has(id)) {
      ...
      console.debug(`Image with id: ${id} is already loaded, returning from cache`);
      return this.cachedTextures.get(id)!;
    }
  ...
  private prepareDataForUpload(image: ImageBitmap | ImageData | {
    width: number;
    height: number;
