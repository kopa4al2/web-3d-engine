...
export default class GLTFParserMainThread {
  ...
  public static async parseGlb(rootDir: string, relativePath: string, textureManager: TextureManager): Promise<GLTFParserMainThread> {
    const fileArrayBuffer = await fetch(rootDir + relativePath, { cache: 'force-cache' })
    ...
    this.glbWorkerPool.addWorker(new Worker(new URL('./workers/GlbImageLoader.worker.ts', import.meta.url), { name: 'GLB-Worker-1' }));
    this.glbWorkerPool.addWorker(new Worker(new URL('./workers/GlbImageLoader.worker.ts', import.meta.url), { name: 'GLB-Worker-2' }));
    this.glbWorkerPool.addWorker(new Worker(new URL('./workers/GlbImageLoader.worker.ts', import.meta.url), { name: 'GLB-Worker-3' }));
    this.glbWorkerPool.addWorker(new Worker(new URL('./workers/GlbImageLoader.worker.ts', import.meta.url), { name: 'GLB-Worker-4' }));

    // this.glbWorkerPool.addWorker(new Worker(new URL('./workers/GlbImageLoader.worker.ts', import.meta.url), { name: 'GLB-Worker-5' }))
    // this.glbWorkerPool.addWorker(new Worker(new URL('./workers/GlbImageLoader.worker.ts', import.meta.url), { name: 'GLB-Worker-6' }))
    // this.glbWorkerPool.addWorker(new Worker(new URL('./workers/GlbImageLoader.worker.ts', import.meta.url), { name: 'GLB-Worker-7' }))
    // this.glbWorkerPool.addWorker(new Worker(new URL('./workers/GlbImageLoader.worker.ts', import.meta.url), { name: 'GLB-Worker-8' }))
  ...
  public static async parseGltf(rootDir: string, gltfPath: string, binaryPath: string, textureManager: TextureManager): Promise<GLTFParserMainThread> {
    const [json, binary] = await Promise.all([
      fetch(rootDir + gltfPath, { cache: 'force-cache' }).then(res => res.json()),
      fetch(rootDir + binaryPath, { cache: 'force-cache' }).then(res => res.arrayBuffer())
    ]);
